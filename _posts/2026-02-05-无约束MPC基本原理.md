...
由此迭代至$Δx(k+p|k)$,即：  
\begin{aligned}
Δx(k+p|k)&=A^pΔx(k)+A^{p-1}B_{u}Δu(k)+A^{p-2}B_{u}Δu(k+1)+......+         
\end{aligned}  

\begin{aligned}
A^{p-m}B_{u}Δu(k+m-1)+A^{p-1}B_{d}Δd(k)          
\end{aligned}  

进一步，可以预测被控输出： 

\begin{aligned}
y_{c}(k+1|k)=C_{c}Δx(k+1|k)+y_{c}(k);
\end{aligned}  

\begin{aligned}
y_{c}(k+2|k)=C_{c}Δx(k+2|k)+y_{c}(k+1|k);
\end{aligned}  

\begin{aligned}
&= (C_c A^2 + C_c A) \Delta x(k) + (C_c A B_u + C_c B_u) \Delta u(k) + 
\end{aligned}  

\begin{aligned}
 C_c B_u \Delta u(k+1) + (C_c A B_d + C_c B_d) \Delta d(k) + y_c(k)
\end{aligned}  
...
\begin{aligned}
+\sum_{i=1}^{p-1} C_c A^{i-1} B_u \Delta u(k+1) + \dots
\end{aligned}  

\begin{aligned}
+\sum_{i=1}^{p-m+1} C_c A^{i-1} B_u \Delta u(k+m-1)
\end{aligned}  

\begin{aligned}
+\sum_{i=1}^p C_c A^{i-1} B_d \Delta d(k) + y_c(k).
\end{aligned}  
