...
$$
\begin{aligned}
\min_z \rho^T \rho, \quad \text{其中}\quad \rho = Az - b.
\end{aligned}
$$

$$
\begin{aligned}
\text{由 } \rho^T \rho &= (Az - b)^T (Az - b) \text{ 的极值条件:} \\
\frac{d\rho^T \rho}{dz} &= 2 \left( \frac{d\rho}{dz} \right)^T \rho = 2A^T (Az - b) \equiv 0 \\
\text{得极值解为 } z^* &= (A^T A)^{-1} A^T b.
\end{aligned}
$$
...
## 优化解的第一个元素作用于系统
这一节将讨论MPC的最后一个步骤，即将优化解的第一个元素作用于系统。
