...
\vdots \\
\sum_{i=1}^{p} C_c A^i
\end{bmatrix}_{p \times 1}, \tag{2} \quad I =
\begin{bmatrix}
I_{n_c \times n_c} \\
I_{n_c \times n_c} \\
\vdots \\
...
$$
\begin{align}
E_p(k+1|k) &= R(k+1) - S_x \Delta x(k) - I y_c(k) - S_d \Delta d(k)\tag{5}
\end{align}
$$
...
$$
\begin{aligned}
\Delta U^*(k) = \left( S_u^T \Gamma_y^T \Gamma_y S_u + \Gamma_u^T \Gamma_u \right)^{-1} S_u^T \Gamma_y^T \Gamma_y E_p (k+1|k)
\end{aligned}
$$
...
$$
\begin{align}
K_{\text{mpc}} = \begin{bmatrix} I_{n_u \times n_u} & \mathbf{0} & \cdots & \mathbf{0} \end{bmatrix}_{1 \times m} \left( S_u^T \Gamma_y^T \Gamma_y S_u + \Gamma_u^T \Gamma_u \right)^{-1} S_u^T \Gamma_y^T \Gamma_y.\tag{6}
\end{align}
$$  
...
$$
\begin{align}
\Delta u(k) = K_{\text{mpc}} E_p (k + 1|k)\tag{7}
\end{align}
$$

其中 $E_p(k+1\vert k)$在线计算。显然，如果加权阵 $\Gamma_y$ 和 $\Gamma_u$ 是与时间无关的常数，则 $K_{\text{mpc}}$ 可以离线计算。在 $k+1$ 时刻，得到新的测量值 $x(k+1)$，将由预测方程重新计算系统未来的输出，并且计算最优控制序列 $\Delta U^*(k+1)$。最后，由预测控制基本原理中的“滚动时域、重复进行”的机制，给出无约束预测控制的算法如下。  

## 算法
(1) 初始化：设定预测时域 $p$ 和控制时域 $m$，初始值 $u(-1) = 0, x(-1) = 0$；计算 $S_x, I, S_d$,$S_u$和计算 $K_{\text{mpc}}$。  

(2) $k \geq 0$ 时刻，得到测量值 $x(k)$ 和 $d(k)$。计算 $y_c(k)$；计算 $\Delta x(k) = x(k) - x(k-1)$。  

(3) 由 (2.28) 计算误差 $E_p(k+1|k)$。  

(4) 由 (2.35) 计算控制量变化量 $\Delta u(k)$。  
