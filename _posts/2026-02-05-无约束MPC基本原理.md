...
\mathbf{0}
\end{bmatrix} \\
E_p(k+1|k) &= R(k+1) - S_x \Delta x(k) - I y_c(k) - S_d \Delta d(k)
\end{aligned}
$$
...
$$
\begin{aligned}
K_{\text{mpc}} = \begin{bmatrix} I_{n_u \times n_u} & \mathbf{0} & \cdots & \mathbf{0} \end{bmatrix}_{1 \times m} \left( S_u^T \Gamma_y^T \Gamma_y S_u + \Gamma_u^T \Gamma_u \right)^{-1} S_u^T \Gamma_y^T \Gamma_y.
\end{aligned}
$$  
...
$$
\begin{aligned}
\Delta u(k) = K_{\text{mpc}} E_p (k + 1|k)
\end{aligned}
$$
