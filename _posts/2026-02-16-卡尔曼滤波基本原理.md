...
请注意，$\hat{x^+}$与$\hat{x^-}$都是对$x_k$的估计。而且都是等于$x_k$的期望，只是条件不一样（后验估计多了一个$k$时刻的量测值）
...
$$
\begin{aligned}
\bar{x_1} = F_{0}\bar{x_{0}} + G_{0}u_{0}
\end{aligned}
$$

又因为$\hat{x}_1^- = E(x_1), \quad \hat{x}_0^+ = E(x_0)$。因此，  
...
考虑到$\hat{x}_1^- = E(x_1), \quad \hat{x}_0^+ = E(x_0)$，上式可写为：
