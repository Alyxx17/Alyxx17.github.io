...
状态转移方程（离散时间，采样间隔 $T = 1s$）：

$\mathbf{x}k = F\mathbf{x}{k-1} + \mathbf{w}{k-1}, \quad \mathbf{w}{k-1} \sim N(0, Q)$

其中状态转移矩阵：

$F = \begin{bmatrix} 1 & T \ 0 & 1 \end{bmatrix} = \begin{bmatrix} 1 & 1 \ 0 & 1 \end{bmatrix}$
...
$z_k = H\mathbf{x}_k + r_k, \quad r_k \sim N(0, R)$
...
即初始位置猜测为0，初始速度猜测为1 m/s。

初始误差协方差矩阵（反映对初始猜测的信任程度）：

$P_0 = \begin{bmatrix} 10 & 0 \ 0 & 5 \end{bmatrix}$

表示位置不确定性较大（方差10），速度也有一定不确定性（方差5），且假设位置和速度的初始估计误差不相关。

3. 第一时刻循环（$k=1$）
3.1 状态预测（先验估计）
根据CV模型预测当前状态：

$\hat{\mathbf{x}}_1^- = F \hat{\mathbf{x}}_0 = \begin{bmatrix} 1 & 1 \ 0 & 1 \end{bmatrix} \begin{bmatrix} 0 \ 1 \end{bmatrix} = \begin{bmatrix} 1 \ 1 \end{bmatrix}$

物理含义：根据上一时刻位置0和速度1，预测1秒后位置变为1，速度保持不变为1。
...
$F P_0 = \begin{bmatrix} 1 & 1 \ 0 & 1 \end{bmatrix} \begin{bmatrix} 10 & 0 \ 0 & 5 \end{bmatrix} = \begin{bmatrix} 10 & 5 \ 0 & 5 \end{bmatrix}$

再乘以 $F^T$：

$(F P_0) F^T = \begin{bmatrix} 10 & 5 \ 0 & 5 \end{bmatrix} \begin{bmatrix} 1 & 0 \ 1 & 1 \end{bmatrix} = \begin{bmatrix} 10+5 & 0+5 \ 0+5 & 0+5 \end{bmatrix} = \begin{bmatrix} 15 & 5 \ 5 & 5 \end{bmatrix}$
...
3.3 卡尔曼增益计算
首先计算创新协方差 $S_1 = H P_1^- H^T + R$：

$H P_1^- = \begin{bmatrix} 1 & 0 \end{bmatrix} \begin{bmatrix} 15 & 5 \ 5 & 5.01 \end{bmatrix} = \begin{bmatrix} 15 & 5 \end{bmatrix}$

$S_1 = (H P_1^-) H^T + R = \begin{bmatrix} 15 & 5 \end{bmatrix} \begin{bmatrix} 1 \ 0 \end{bmatrix} + 10 = 15 + 10 = 25$

卡尔曼增益 $K_1 = P_1^- H^T S_1^{-1}$：

先计算 $P_1^- H^T$：

$P_1^- H^T = \begin{bmatrix} 15 & 5 \ 5 & 5.01 \end{bmatrix} \begin{bmatrix} 1 \ 0 \end{bmatrix} = \begin{bmatrix} 15 \ 5 \end{bmatrix}$

则：

$K_1 = \begin{bmatrix} 15 \ 5 \end{bmatrix} \times \frac{1}{25} = \begin{bmatrix} 0.6 \ 0.2 \end{bmatrix}$

物理含义：增益的第一项0.6表示对位置测量的信任程度中等（因预测位置不确定性15，测量噪声10）；第二项0.2表示通过位置测量可以间接修正速度估计，这正是CV模型能估计速度的关键。

3.4 状态更新
假设第一次测量值 $z_1 = 3$（真实位置未知，但传感器读数为3）。

先计算测量残差（创新）：

$\tilde{y}_1 = z_1 - H \hat{\mathbf{x}}_1^- = 3 - \begin{bmatrix} 1 & 0 \end{bmatrix} \begin{bmatrix} 1 \ 1 \end{bmatrix} = 3 - 1 = 2$

后验状态估计：

$\hat{\mathbf{x}}_1 = \hat{\mathbf{x}}_1^- + K_1 \tilde{y}_1 = \begin{bmatrix} 1 \ 1 \end{bmatrix} + \begin{bmatrix} 0.6 \ 0.2 \end{bmatrix} \times 2 = \begin{bmatrix} 1 + 1.2 \ 1 + 0.4 \end{bmatrix} = \begin{bmatrix} 2.2 \ 1.4 \end{bmatrix}$

物理含义：位置从预测的1修正为2.2，更接近测量值3；速度从预测的1修正为1.4，因为测量值大于预测，暗示实际速度可能比预测的快。
...
$K_1 H = \begin{bmatrix} 0.6 \ 0.2 \end{bmatrix} \begin{bmatrix} 1 & 0 \end{bmatrix} = \begin{bmatrix} 0.6 & 0 \ 0.2 & 0 \end{bmatrix}$

则 $I - K_1 H$：

$I - K_1 H = \begin{bmatrix} 1 & 0 \ 0 & 1 \end{bmatrix} - \begin{bmatrix} 0.6 & 0 \ 0.2 & 0 \end{bmatrix} = \begin{bmatrix} 0.4 & 0 \ -0.2 & 1 \end{bmatrix}$

最后计算 $P_1$：

$P_1 = \begin{bmatrix} 0.4 & 0 \ -0.2 & 1 \end{bmatrix} \begin{bmatrix} 15 & 5 \ 5 & 5.01 \end{bmatrix} = \begin{bmatrix} 0.4 \times 15 + 0 \times 5 & 0.4 \times 5 + 0 \times 5.01 \ -0.2 \times 15 + 1 \times 5 & -0.2 \times 5 + 1 \times 5.01 \end{bmatrix}$

$P_1 = \begin{bmatrix} 6 & 2 \ -3 + 5 & -1 + 5.01 \end{bmatrix} = \begin{bmatrix} 6 & 2 \ 2 & 4.01 \end{bmatrix}$

物理含义：位置不确定性从15降至6，速度不确定性从5.01降至4.01，且位置和速度的估计误差产生正相关（协方差2），这符合物理直觉——位置偏大时速度也倾向于偏大。

4. 第二时刻循环（$k=2$）
4.1 状态预测
利用上一时刻后验估计 $\hat{\mathbf{x}}_1 = [2.2, 1.4]^T$：

$\hat{\mathbf{x}}_2^- = F \hat{\mathbf{x}}_1 = \begin{bmatrix} 1 & 1 \ 0 & 1 \end{bmatrix} \begin{bmatrix} 2.2 \ 1.4 \end{bmatrix} = \begin{bmatrix} 3.6 \ 1.4 \end{bmatrix}$

物理含义：预测1秒后位置变为2.2 + 1.4 = 3.6，速度保持1.4。
...
$F P_1 = \begin{bmatrix} 1 & 1 \ 0 & 1 \end{bmatrix} \begin{bmatrix} 6 & 2 \ 2 & 4.01 \end{bmatrix} = \begin{bmatrix} 6+2 & 2+4.01 \ 2 & 4.01 \end{bmatrix} = \begin{bmatrix} 8 & 6.01 \ 2 & 4.01 \end{bmatrix}$

再乘以 $F^T$：

$(F P_1) F^T = \begin{bmatrix} 8 & 6.01 \ 2 & 4.01 \end{bmatrix} \begin{bmatrix} 1 & 0 \ 1 & 1 \end{bmatrix} = \begin{bmatrix} 8+6.01 & 0+6.01 \ 2+4.01 & 0+4.01 \end{bmatrix} = \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.01 \end{bmatrix}$

加上过程噪声 $Q$：

$P_2^- = \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.01 \end{bmatrix} + \begin{bmatrix} 0 & 0 \ 0 & 0.01 \end{bmatrix} = \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.02 \end{bmatrix}$

4.3 卡尔曼增益计算
创新协方差：

$H P_2^- = \begin{bmatrix} 1 & 0 \end{bmatrix} \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.02 \end{bmatrix} = \begin{bmatrix} 14.01 & 6.01 \end{bmatrix}$

$S_2 = (H P_2^-) H^T + R = 14.01 + 10 = 24.01$

卡尔曼增益：

先计算 $P_2^- H^T$：

$P_2^- H^T = \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.02 \end{bmatrix} \begin{bmatrix} 1 \ 0 \end{bmatrix} = \begin{bmatrix} 14.01 \ 6.01 \end{bmatrix}$

则：

$K_2 = \begin{bmatrix} 14.01 \ 6.01 \end{bmatrix} \times \frac{1}{24.01} = \begin{bmatrix} 0.583 \ 0.250 \end{bmatrix}$

物理含义：增益相比第一时刻略有变化，因为预测不确定性已经调整。
...
$\tilde{y}_2 = z_2 - H \hat{\mathbf{x}}_2^- = 4.5 - 3.6 = 0.9$

后验状态估计：

$\hat{\mathbf{x}}_2 = \hat{\mathbf{x}}_2^- + K_2 \tilde{y}_2 = \begin{bmatrix} 3.6 \ 1.4 \end{bmatrix} + \begin{bmatrix} 0.583 \ 0.250 \end{bmatrix} \times 0.9$

$\hat{\mathbf{x}}_2 = \begin{bmatrix} 3.6 + 0.5247 \ 1.4 + 0.225 \end{bmatrix} = \begin{bmatrix} 4.1247 \ 1.625 \end{bmatrix}$

物理含义：位置从预测的3.6修正为4.1247，速度从1.4修正为1.625。通过两次循环，尽管仅测量位置，速度估计已从初始的1逐步调整，开始跟踪真实运动趋势。
...
$K_2 H = \begin{bmatrix} 0.583 \ 0.250 \end{bmatrix} \begin{bmatrix} 1 & 0 \end{bmatrix} = \begin{bmatrix} 0.583 & 0 \ 0.250 & 0 \end{bmatrix}$

$I - K_2 H = \begin{bmatrix} 0.417 & 0 \ -0.250 & 1 \end{bmatrix}$

则：

$P_2 = \begin{bmatrix} 0.417 & 0 \ -0.250 & 1 \end{bmatrix} \begin{bmatrix} 14.01 & 6.01 \ 6.01 & 4.02 \end{bmatrix}$

先计算第一行：

$0.417 \times 14.01 + 0 \times 6.01 = 5.842$
$0.417 \times 6.01 + 0 \times 4.02 = 2.506$

再计算第二行：

$-0.250 \times 14.01 + 1 \times 6.01 = -3.5025 + 6.01 = 2.5075$
$-0.250 \times 6.01 + 1 \times 4.02 = -1.5025 + 4.02 = 2.5175$

因此：

$P_2 = \begin{bmatrix} 5.842 & 2.506 \ 2.508 & 2.518 \end{bmatrix}$

物理含义：不确定性继续降低，位置方差从14.01降至5.842，速度方差从4.02降至2.518，且两者仍保持正相关。
