...
$$\mathbf{x}_k = F\mathbf{x}_{k-1} + \mathbf{w}_{k-1}, \quad \mathbf{w}_{k-1} \sim N(0, Q)$$
...
$$y_k = H\mathbf{x}_k + r_k, \quad v_k \sim N(0, R)$$
...
$$y_1 - H \hat{\mathbf{x}}_1^- = 3 - \begin{bmatrix} 1 & 0 \end{bmatrix} \begin{bmatrix} 1 \\ 1 \end{bmatrix} = 3 - 1 = 2$$

后验状态估计：

$$\hat{\mathbf{x}}_1^+ = \hat{\mathbf{x}}_1^- + K_1 (y_1 - H \hat{\mathbf{x}}_1^-） = \begin{bmatrix} 1 \\ 1 \end{bmatrix} + \begin{bmatrix} 0.6 \\ 0.2 \end{bmatrix} \times 2 =  \begin{bmatrix} 2.2 \\ 1.4 \end{bmatrix}$$
...
然后利用$P_1^+ $和$\hat x_1^+$作为$k=2$时刻的初始条件，经过简单计算后$k=2$时位置从预测的$3.6$修正为$4.1247$，速度从$1.4$修正为$1.625$。通过两次循环，尽管仅测量位置，速度估计已从初始的$1$逐步调整，开始跟踪真实运动趋势。
