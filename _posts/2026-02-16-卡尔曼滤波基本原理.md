...
我们选取$K_k$的最优标准是使$k$时刻的估计误差的方差和最小。（实际上，由这个最优标准推导出的 $K_k$ 自然能保证矩阵 $(I - K_kH_k)$ 的谱半径小于 $1$）那么：  
...
定义$P_k$为估计误差的协方差矩阵。实际上，$P_k$的结构就是$k$时刻误差$\varepsilon_{x,k}$的自相关矩阵，又因为是无偏估计，误差的期望为$0$，因此计算协方差矩阵的公式中只剩下自相关矩阵。  
...
$$
\begin{align}
\bar{x_k} = E(\bar{x_k}) = F_{k-1}\bar{x_{k-1}} + G_{k-1}u_{k-1}\tag{1}
\end{align}\
$$
...
上式的期望即$x_k$的协方差，考虑到$(x_{k-1}-\bar{x}_{k-1})$与$w_{k-1}$不相关，因此，  
