...
又因为$\hat{x}_1^- = E(x_1)=\bar x_{1}, \quad \hat{x}_0^+ = E(x_0)= \bar x_{0}$。因此，  
...
考虑到$\hat{x}_1^- = E(x_1)=\bar x_{1}, \quad \hat{x}_0^+ = E(x_0)= \bar x_{0}$以及$P_k^-$和$P_k^+$的表达式，上式可写为：
...
$$
\begin{align}
P_{k}^- = F_{k-1}P_{k-1}^+F_{k-1}^T + Q_{k-1}tag{4}
\end{align}
$$
